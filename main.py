...
branch_sales = fil_df.groupby('Outlet')['Revenue'].sum()
...
# Calculate the total sales for each branch in comparison
Branch_c= fil_df_c.groupby('Outlet')['Revenue'].sum()

# Merge the DataFrames based on the 'ID' column
merged_branches = pd.merge(branch_sales, Branch_c, on='Outlet', how='outer')

# Replace None values in 'ColumnA' with zero
merged_branches['Revenue_x'].fillna(0, inplace=True)
merged_branches['Revenue_y'].fillna(0, inplace=True)

#Calculate variance
merged_branches['Variance'] = merged_branches['Revenue_x'] - merged_branches['Revenue_y']
sorted_b = merged_branches.sort_values(by='Revenue_x', ascending=False)
#Display the diagnostic Dataframe
st.write('''
Overview of branch sales vs comparison time period
''')
st.dataframe(sorted_b)

# Create a new DataFrame with only positive variance items
negative_variance_df = sorted_b[sorted_b['Variance'] < 0]

st.write('''
These stores' sales dropped in the chosen time period
''')
st.dataframe(negative_variance_df)

# Create a new DataFrame with only negative revenue stores
positive_variance_df = sorted_b[sorted_b['Variance'] >= 0]

st.write('''
These stores' sales increased or remained stable in chosen time period
''')
...
Salesmix Overview
''')
st.dataframe(merged_Salesmix.sort_values(by='Revenue_x', ascending=False))
...
''')
st.dataframe(negative_variance_df.sort_values(by='Revenue_x', ascending=False))
...
''')
st.dataframe(positive_variance_df.sort_values(by='Revenue_x', ascending=False))
