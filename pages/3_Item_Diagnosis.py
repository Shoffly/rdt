...
# Branch selection
branch_select = df['Items'].unique()
store = st.sidebar.selectbox("Pick the item", branch_select)
...
fil_df = df[(df['Date'] >= sd) & (df['Date'] <= ed) & (df['Items'].isin([store]))]
...
fil_df_c = df[(df['Date'] >= sd_c) & (df['Date'] <= ed_c) & (df['Items'].isin([store]))]
#page contents
st.title(f"{store} Analysis")
# Display the value in the subheader using Markdown formatting
st.write(f" This is a deep dive on {store} for the dates between {sd} and {ed}.")
...
Sales_mix= fil_df.groupby('Outlet')['Revenue'].sum()
...
''')
# Calculate the total sales for each item
Sales_mix_c= fil_df_c.groupby('Outlet')['Revenue'].sum()

# Merge the DataFrames based on the 'ID' column
merged_Salesmix = pd.merge(Sales_mix, Sales_mix_c, on='Outlet', how='outer')
...
st.write(f'''
These branches sales dropped for {store}
''')
...
st.write(f'''
These branches sales increased or remained stable for {store}
''')
